...
class Vector:
    def __init__(self, c1: [float, Point] = None,
    ...
    def scalar_product(self, vector: 'Vector') -> [int, float]:
    ...
    def __mul__(self, object: [int, float, 'Vector']):
    ...
    def __rmul__(self, object: [int, float, 'Vector']):
...
class Camera:
    def __init__(self, position: Point, look_dir_at: [Vector, Point], 
                 fov: float, k: float,
                 draw_distance: float):
